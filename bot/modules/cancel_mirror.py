...
bot.add_handler(MessageHandler(cancel_mirror, filters=regex(
    f"^/{BotCommands.CancelMirror}(_\w+)?(?!all)") & (CustomFilters.authorized | CustomFilters.sudo_user)))
bot.add_handler(MessageHandler(cancell_all_buttons, filters=command(
    BotCommands.CancelAllCommand) & (CustomFilters.authorized | CustomFilters.sudo_user)))
